...
import type { LocationResult, WeatherMetric, ChartType } from './types'
import { METRICS } from './locations'
import { aggregateMonthly, aggregateDaily } from './api'
...
export default function Chart({ results, metric, chartType, onToggleChartType }: ChartProps) {
  ...
  if (chartType === 'line') {
    ...
    const chartData = dates.map((date, i) => {
      ...
      for (const lw of locationWeekly) {
        row[lw.location.name] = lw.data[i]?.value ?? 0
      }
      return row
    })
...
function ChartHeader({ metricInfo, isCountMetric, chartType, onToggle, isLine }: {
